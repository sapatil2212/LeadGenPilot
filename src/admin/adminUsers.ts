...
import { allowedPlanKeys, assertPlanKey } from "./planCatalog";
...
function toRow(u: any, revenueByUser: Map<string, number>) {
  const entitlements = getEntitlements(u.plan);
...
router.get(
  ...
  adminRoute(async (req, res) => {
    ...
    res.json({
      rows: users.map((u) => toRow(u, revenue)),
...
router.get(
  ...
  adminRoute(async (req, res) => {
    ...
    const revenue = await revenueFor(users.map((u) => u.id));
    const rows = users.map((u) => toRow(u, revenue));
...
router.post(
  ...
  adminRoute(async (req, res) => {
    ...
    res.status(201).json({ success: true, user: toRow({ ...user, _count: {}, subscriptions: [] }, new Map()) });
  })
);
...
router.patch(
  ...
  adminRoute(async (req, res) => {
    ...
    res.json({ success: true, user: toRow(updated, await revenueFor([id])) });
  })
);
