...
/**
 * A published plan, exactly as GET /api/plans serves it.
 *
 * That endpoint reads the catalogue an operator edits at
 * /superadmin/dashboard/plans, so the upgrade modal below shows current pricing
 * instead of a hardcoded copy that silently drifts from it.
 */
...
function formatPlanPrice(minorUnits: number, currency: string): string {
  ...
  try {
    ...
    // An unrecognised currency code must not break the pricing modal.
    return `${currency} ${major.toLocaleString()}`;
  }
}

/**
 * Builds the feature checklist for a plan card.
 *
 * Quota and seats come from the plan's own numbers; the gated features are
 * listed for every plan in a stable order, on or off, so the cards compare.
 */
function planFeatureRows(
  ...
  return [
    ...
    { label: plan.seats === null ? "Unlimited seats" : `${plan.seats} seat${plan.seats === 1 ? "" : "s"}`, on: true },
    ...(plan.trialDays > 0 ? [{ label: `${plan.trialDays}-day free trial`, on: true }] : []),
    ...featureKeys.map((key) => ({
      label: featureLabels[key] || key,
      on: plan.features.includes(key),
    })),
  ];
}
...
export default function App({ currentUser, currentWorkspace, entitlements, usage, onLogout, onRefreshAccount }: AppProps = {}) {
  ...
  // The stable plan key, used to mark the active plan in the pricing modal.
  // Display names are operator-editable, so they cannot identify a plan.
  const currentPlanKey = (currentUser?.plan || "").toLowerCase();
  ...
  // Published plans for the upgrade modal, loaded from the operator's catalogue.
  const [pricingPlans, setPricingPlans] = useState<PublicPlan[]>([]);
  const [pricingFeatureKeys, setPricingFeatureKeys] = useState<string[]>([]);
  const [pricingFeatureLabels, setPricingFeatureLabels] = useState<Record<string, string>>({});
  ...
  /*
   * Load the published plans when the upgrade modal opens.
   *
   * Fetched on open rather than at mount so the dashboard costs nothing for the
   * users who never open it, and so an operator's price change is picked up
   * without the customer reloading the app.
   */
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPricingFeatureKeys(Array.isArray(data.featureKeys) ? data.featureKeys : []);
        setPricingFeatureLabels(data.featureLabels && typeof data.featureLabels === "object" ? data.featureLabels : {});
      } catch (err) {
        if ((err as Error).name === "AbortError") return;
        setPricingError("Could not load the current plans. Please try again.");
      } finally {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape" && headerSearchOpen) {
        setHeaderSearchOpen(false);
      }
    };
  ...
  useEffect(() => {
    if (headerSearchOpen && headerSearchInputRef.current) {
    ...
    <div className={`relative h-dvh ${bgMain} font-sans overflow-hidden`}>
      ...
      <aside className={`${sidebarCollapsed ? "w-[72px]" : "w-64"} ${bgAside} fixed inset-y-0 left-0 z-30 flex h-dvh flex-col justify-between overflow-hidden transition-[width] duration-200 shadow-xl`}>
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          {/* Logo Brand */}
          <div className={`h-14 flex items-center justify-center ${sidebarCollapsed ? "px-2" : "px-4"} border-b ${isLight ? "border-slate-200/80" : "border-[#1e293b]/80"} shrink-0`}>
            <div className="relative flex items-center justify-center">
              <img
                src={isLight ? "/assets/logo/logo.png" : "/assets/logo/logo-dark.png"}
                alt="LeadGenPilot"
                className={`${sidebarCollapsed ? "h-6 max-w-[48px]" : "h-8 max-w-[170px]"} w-auto object-contain transition-all duration-200`}
              />
            </div>
          </div>

          {/* Route-aware navigation — grouped for fast scanning. */}
          <nav className="min-h-0 flex-1 overflow-y-auto px-2.5 py-2 space-y-2 no-scrollbar scrollbar-none [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden" aria-label="Dashboard navigation">
            {DASHBOARD_GROUPS.map((group) => {
              ...
              return (
                <div key={group.id} className="space-y-0.5">
                  {sidebarCollapsed ? (
                    group.id !== "overview" && (
                      <div className={`h-px mx-1.5 my-1 ${isLight ? "bg-slate-200" : "bg-slate-800/80"}`} />
                    )
                  ) : (
                    <div className={`px-2 pt-1 pb-0.5 text-[9px] font-semibold uppercase tracking-[0.14em] ${
                    ...
                    {routes.map((route) => {
                      ...
                      return (
                        <a
                          ...
                          className={`btn-interactive group relative w-full flex items-center rounded-lg text-xs font-medium transition-all duration-150 cursor-pointer ${
                            sidebarCollapsed ? "justify-center px-0 py-2" : "justify-between px-2.5 py-1.5"
                          } ${
                            active
                              ? isLight
                                ? "bg-indigo-50/90 text-indigo-700 font-semibold border border-indigo-200/70 shadow-xs"
                        ...
                        >
                          ...
                          {active && !sidebarCollapsed && (
                            <span className="absolute left-0 top-1 bottom-1 w-0.5 rounded-r-full bg-indigo-500" />
                          )}
                          <span className={`flex items-center ${sidebarCollapsed ? "" : "gap-2.5"}`}>
                            <Icon className={`h-4 w-4 shrink-0 transition-transform duration-150 group-hover:scale-105 ${active ? "text-indigo-500" : isLight ? "text-slate-400" : "text-slate-500"}`} />
                            {!sidebarCollapsed && <span className="truncate">{route.label}</span>}
                          </span>
                          {!sidebarCollapsed && badge !== undefined && (
                            <span className={`text-[9.5px] px-1.5 py-0.5 rounded font-semibold tabular-nums leading-none ${
                              route.id === "conversations"
                                ? "bg-indigo-500 text-white"
                                : isLight
                                  ? "bg-slate-100 text-slate-600 border border-slate-200"
                                  : "bg-[#1e293b] text-slate-400 border border-slate-700/50"
                            }`}>
                              {badge}
                            </span>
                          )}
                          ...
                          {sidebarCollapsed && pulse && (
                            <span className="absolute top-1 right-1.5 h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
                          )}
                        </a>
                      );
                    })}
        ...
        <div className={`border-t ${isLight ? "border-slate-200" : "border-[#1e293b]/60"} ${sidebarCollapsed ? "p-2" : "p-3"} space-y-2 shrink-0`}>
          {!sidebarCollapsed && (
            <div className={`p-2.5 rounded-lg border ${isLight ? "bg-slate-50/70 border-slate-200/80" : "bg-[#040711] border-[#1e293b]/80"} space-y-1.5`}>
              <div className="flex items-center justify-between">
                <span className={`text-[9px] font-semibold uppercase tracking-wider ${isLight ? "text-slate-400" : "text-slate-500"}`}>Current Plan</span>
                {entitlements ? (
                  isFreePlan ? (
                    <span className={`text-[9.5px] px-1.5 py-0.5 rounded font-semibold border ${isLight ? "bg-white text-slate-600 border-slate-200" : "bg-slate-800/80 text-slate-300 border-slate-700"}`}>
                      {entitlements.planName}
                    </span>
                  ) : (
                    <span className="text-[9.5px] inline-flex items-center gap-1 text-white bg-gradient-to-r from-indigo-600 to-violet-600 px-2 py-0.5 rounded-full font-semibold shadow-xs">
                      <Sparkles className="h-2.5 w-2.5" /> {entitlements.planName}
                    </span>
                  )
                ) : (
                  <span className="text-[9.5px] text-indigo-500 bg-indigo-500/10 px-1.5 py-0.5 rounded font-semibold border border-indigo-500/20">Live</span>
                )}
              </div>
              {usage && !usage.unlimited && usage.limit !== null && (
                <div title={`${usage.used} of ${usage.limit} leads used this month`}>
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-[9.5px] font-semibold tabular-nums ${isLight ? "text-slate-600" : "text-slate-400"}`}>
                      {usage.used} / {usage.limit} leads
                    </span>
                    <span className="text-[9.5px] text-slate-500 font-semibold tabular-nums">
          ...
          <button
            ...
            className={`w-full btn-interactive flex items-center gap-1.5 rounded-lg text-[11px] font-medium cursor-pointer transition-all ${
              sidebarCollapsed ? "justify-center py-1.5" : "justify-center py-1.5"
          ...
          >
            {sidebarCollapsed ? <ChevronRight className="h-3.5 w-3.5" /> : <><ChevronLeft className="h-3.5 w-3.5" /> Collapse rail</>}
          </button>
        </div>
      </aside>
      ...
      <div className="min-w-0">
        ...
        <header className={`${sidebarCollapsed ? "left-[72px]" : "left-64"} fixed top-0 right-0 h-14 ${bgHeader} px-3 sm:px-5 lg:px-6 flex items-center justify-between z-40 backdrop-blur-md transition-[left] duration-200 border-b ${borderSubtle}`}>
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            {/* Animated Search Trigger & Expandable Searchbar */}
            <div ref={headerSearchRef} className="relative">
              {!headerSearchOpen ? (
                <button
                  onClick={() => setHeaderSearchOpen(true)}
                  className={`btn-interactive group flex items-center gap-2 px-2.5 sm:px-3 py-1.5 rounded-lg border text-xs font-medium cursor-pointer transition-all duration-200 shadow-xs ${
                    isLight
                      ? "bg-slate-50 hover:bg-white border-slate-200 text-slate-600 hover:text-slate-900 hover:border-indigo-300"
                      : "bg-slate-900/60 hover:bg-slate-900 border-[#1e293b] text-slate-300 hover:text-white hover:border-indigo-500/40"
                  }`}
                  title="Quick Search (Ctrl + K)"
                >
                  <span className="relative flex items-center justify-center">
                    <Search className="h-3.5 w-3.5 text-indigo-500 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-12 animate-pulse" />
                    <span className="absolute -top-0.5 -right-0.5 flex h-1.5 w-1.5">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500"></span>
                    </span>
                  </span>
                  <span className="hidden sm:inline text-slate-400 font-normal group-hover:text-slate-300">Search...</span>
                  <kbd className={`hidden md:inline-flex items-center px-1.5 py-0.2 text-[9px] font-mono font-semibold rounded border ${
                    isLight ? "bg-white text-slate-400 border-slate-200" : "bg-slate-800 text-slate-400 border-slate-700/60"
                  }`}>
                    ⌘K
                  </kbd>
                </button>
                ...
                <div className="relative animate-scaleUp">
                  <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border w-72 sm:w-80 shadow-lg transition-all ${
                    isLight
                      ? "bg-white border-indigo-500/60 ring-2 ring-indigo-500/15"
                      : "bg-[#090d16] border-indigo-500/60 ring-2 ring-indigo-500/20"
                  }`}>
                    <Search className="h-3.5 w-3.5 text-indigo-500 shrink-0 animate-pulse" />
                    <input
                      ref={headerSearchInputRef}
                      type="text"
                      value={headerSearchQuery}
                      onChange={(e) => setHeaderSearchQuery(e.target.value)}
                      placeholder="Search routes, leads, tools..."
                    ...
                    {headerSearchQuery ? (
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-slate-200 text-[10px] font-mono px-1 py-0.2 rounded border border-slate-700/60 cursor-pointer"
                        title="Close (Esc)"
                      >
                        ESC
                      </button>
                  ...
                  <div className={`absolute right-0 top-10 w-80 sm:w-96 rounded-xl shadow-2xl border overflow-hidden z-[110] transition-all p-1.5 space-y-2 ${
                    isLight
                      ? "bg-white border-slate-200 text-slate-800"
                      : "bg-[#090d16] border-[#1e293b] text-slate-100"
                  }`}>
                    {/* Navigation Pages Matches */}
                    <div>
                      <div className={`px-2 py-1 text-[9px] font-bold uppercase tracking-wider ${isLight ? "text-slate-400" : "text-slate-500"}`}>
                        Navigation
                      </div>
                      <div className="space-y-0.5">
                        {DASHBOARD_ROUTES.filter(r => 
                          !headerSearchQuery || 
                          r.label.toLowerCase().includes(headerSearchQuery.toLowerCase()) || 
                          r.description.toLowerCase().includes(headerSearchQuery.toLowerCase())
                        ).slice(0, 4).map(route => {
                          const RouteIcon = DASHBOARD_ROUTE_ICONS[route.id];
                          return (
                            <button
                              key={route.id}
                              onClick={() => {
                              ...
                              className={`w-full flex items-center justify-between px-2.5 py-1.5 text-xs rounded-lg text-left cursor-pointer transition-colors ${
                                isLight ? "hover:bg-indigo-50 text-slate-700 hover:text-indigo-700" : "hover:bg-indigo-950/40 text-slate-300 hover:text-indigo-300"
                              }`}
                            >
                              <div className="flex items-center gap-2 truncate">
                                <RouteIcon className="h-3.5 w-3.5 text-indigo-400 shrink-0" />
                                <span className="font-semibold truncate">{route.label}</span>
                                <span className="text-[10px] text-slate-500 truncate hidden sm:inline">{route.title}</span>
                              </div>
                    ...
                    {/* Leads Matches */}
                    {headerSearchQuery && (
                      <div className="border-t pt-1.5 border-slate-200 dark:border-slate-800/80">
                        <div className={`px-2 py-1 text-[9px] font-bold uppercase tracking-wider ${isLight ? "text-slate-400" : "text-slate-500"}`}>
                          Matching Leads
                        </div>
                        <div className="space-y-0.5 max-h-40 overflow-y-auto no-scrollbar">
                          {processedLeads.filter(l => 
                            l.businessName.toLowerCase().includes(headerSearchQuery.toLowerCase()) ||
                            (l.category && l.category.toLowerCase().includes(headerSearchQuery.toLowerCase())) ||
                            l.phone.includes(headerSearchQuery) ||
                            l.address.toLowerCase().includes(headerSearchQuery.toLowerCase())
                          ).slice(0, 4).map(lead => (
                            <button
                              key={lead.id}
                              onClick={() => {
                                setSearchTerm(lead.businessName);
                                navigateToTab("leads");
                                setHeaderSearchOpen(false);
                                setHeaderSearchQuery("");
                              }}
                            ...
                            >
                              <div className="truncate">
                                <div className="font-semibold truncate">{lead.businessName}</div>
                                <div className="text-[10px] text-slate-500 truncate">{lead.category || "Lead"} · {lead.address || lead.phone}</div>
                              </div>
                              <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase ml-2 ${
            ...
            return (
              <div className="space-y-6 animate-fadeIn pb-8">
                ...
                <div className={`p-4 sm:p-5 border rounded-2xl relative overflow-hidden flex flex-col lg:flex-row lg:items-center justify-between gap-4 transition-all ${
                  isLight 
                    ? "bg-gradient-to-r from-white via-indigo-50/20 to-white border-slate-200" 
                    : "bg-gradient-to-r from-indigo-950/20 via-[#0a0f1d] to-slate-900 border-[#1e293b]"
                }`}>
                  ...
                  <div className="flex items-center gap-2 shrink-0 relative z-10 flex-wrap">
                    <button
                      ...
                      className="btn-interactive flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-indigo-600 via-indigo-500 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white text-xs font-bold rounded-lg cursor-pointer transition-all"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
                  ...
                  <div 
                    ...
                    className={`group cursor-pointer rounded-xl p-3.5 border transition-all duration-200 relative overflow-hidden hover:-translate-y-0.5 ${
                      isLight 
                        ...
                        : "bg-gradient-to-br from-slate-900/90 to-[#0b101d] border-[#1e293b] hover:border-indigo-500/40"
                    }`}
                  >
                    ...
                    <div className="flex justify-between items-start relative z-10">
                      ...
                      <div className={`p-2 rounded-lg transition-transform duration-200 group-hover:scale-110 ${
                        isLight ? "bg-indigo-50 text-indigo-600 border border-indigo-100" : "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20"
                      }`}>
                  ...
                  <div 
                    ...
                    className={`group cursor-pointer rounded-xl p-3.5 border transition-all duration-200 relative overflow-hidden hover:-translate-y-0.5 ${
                      isLight 
                        ...
                        : "bg-gradient-to-br from-slate-900/90 to-[#0b101d] border-[#1e293b] hover:border-rose-500/40"
                    }`}
                  >
                    ...
                    <div className="flex justify-between items-start relative z-10">
                      ...
                      <div className={`p-2 rounded-lg transition-transform duration-200 group-hover:scale-110 ${
                        isLight ? "bg-rose-50 text-rose-600 border border-rose-100" : "bg-rose-500/10 text-rose-400 border border-rose-500/20"
                      }`}>
                  ...
                  <div 
                    ...
                    className={`group cursor-pointer rounded-xl p-3.5 border transition-all duration-200 relative overflow-hidden hover:-translate-y-0.5 ${
                      isLight 
                        ...
                        : "bg-gradient-to-br from-slate-900/90 to-[#0b101d] border-[#1e293b] hover:border-amber-500/40"
                    }`}
                  >
                  ...
                  {/* Card 4: Webhook & Outbox Sync (Interactive Google Sheet Card) */}
                  <div 
                    onClick={() => {
                    ...
                    role="button"
                    tabIndex={0}
                    title="Click to view CRM sync and Google Sheets status"
                    className={`group cursor-pointer rounded-xl p-3.5 border transition-all duration-200 relative overflow-hidden hover:-translate-y-0.5 ${
                      isLight 
                        ? "bg-white border-slate-200 hover:border-emerald-300" 
                        : "bg-gradient-to-br from-slate-900/90 to-[#0b101d] border-[#1e293b] hover:border-emerald-500/40"
                    }`}
                  >
                    <div className="absolute top-0 right-0 -mr-4 -mt-4 w-20 h-20 bg-emerald-500/10 rounded-full blur-lg group-hover:bg-emerald-500/20 transition-all duration-300"></div>
                    <div className="flex justify-between items-start relative z-10">
                      <div>
                        <span className={`text-[10px] font-bold tracking-wider uppercase ${failedLeads.length > 0 ? "text-amber-500" : "text-emerald-500"}`}>
                          Outbox Delivery
                        </span>
                        <div className="mt-0.5 flex items-baseline gap-1.5">
                          ...
                          <span className={`text-[10px] font-semibold ${failedLeads.length > 0 ? "text-amber-500" : "text-emerald-600"}`}>
                    ...
                    <div className="mt-3 pt-2 border-t border-dashed border-slate-700/20 flex items-center justify-between text-[10px] relative z-10">
                      {failedLeads.length > 0 ? (
                        <button 
                          ...
                          className="font-bold text-amber-500 hover:text-amber-600 underline cursor-pointer border-0 bg-transparent p-0 flex items-center gap-1"
                        >
                        ...
                        <>
                          ...
                          <span className="inline-flex items-center gap-1 font-bold text-emerald-500 group-hover:translate-x-0.5 transition-transform">
                            Connected <ArrowRight className="h-2.5 w-2.5" />
                          </span>
                        </>
                      )}
                    </div>
                  ...
                  <div className={`lg:col-span-8 ${
                          ...
                          <button
                            ...
                            className={`px-2 py-0.5 text-[10px] font-semibold rounded-md transition-all cursor-pointer ${
                              overviewMapFilter === "ALL"
                                ? isLight ? "bg-white text-indigo-600 border border-slate-200 font-bold" : "bg-indigo-600 text-white font-bold"
                          ...
                          <button
                            ...
                            className={`px-2 py-0.5 text-[10px] font-semibold rounded-md transition-all cursor-pointer ${
                              overviewMapFilter === "COLD"
                                ? isLight ? "bg-slate-700 text-white font-bold" : "bg-slate-600 text-white font-bold"
                    ...
                    {processedLeads.length === 0 ? (
                      <div className={`flex-grow flex flex-col items-center justify-center border border-dashed ${
                        ...
                        <button
                          onClick={() => navigateToTab("finder")}
                          className="mt-3 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg cursor-pointer btn-interactive"
                        >
                          Open Lead Finder
                        </button>
                      </div>
                    ...
                    <div className={`mt-3 pt-2.5 border-t flex flex-wrap items-center justify-between gap-2.5 text-[10px] ${
                      ...
                      <span className="text-[9.5px] text-indigo-500 font-medium italic">
                  ...
                  <div className="lg:col-span-4 space-y-4">
                    ...
                    <div className={`${
                      ...
                      <div className="space-y-3">
                        <div className={`h-2 w-full rounded-full overflow-hidden flex p-0.5 border ${
                          isLight ? "bg-slate-100 border-slate-200" : "bg-slate-800/40 border-slate-700/40"
                        }`}>
                          <div 
                          ...
                          />
                          <div 
                            className="h-full bg-slate-400 transition-all duration-700 rounded-r-full" 
                        ...
                        <div className="space-y-2 pt-1">
                          ...
                          >
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-[11px] font-bold ${isLight ? "text-rose-700" : "text-rose-400"}`}>HOT Priority</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <span className={`text-xs font-black tabular-nums ${isLight ? "text-slate-900" : "text-white"}`}>{hotLeads}</span>
                              <span className={`text-[9.5px] ${isLight ? "text-slate-500" : "text-slate-400"}`}>({hotPercentage}%)</span>
                            </div>
                          </div>
                          ...
                          >
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-[11px] font-bold ${isLight ? "text-amber-700" : "text-amber-400"}`}>WARM Priority</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <span className={`text-xs font-black tabular-nums ${isLight ? "text-slate-900" : "text-white"}`}>{warmLeads}</span>
                              <span className={`text-[9.5px] ${isLight ? "text-slate-500" : "text-slate-400"}`}>({warmPercentage}%)</span>
                            </div>
                          </div>
                          ...
                          >
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-[11px] font-bold ${isLight ? "text-slate-700" : "text-slate-400"}`}>COLD Priority</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <span className={`text-xs font-black tabular-nums ${isLight ? "text-slate-900" : "text-white"}`}>{coldLeads}</span>
                              <span className={`text-[9.5px] ${isLight ? "text-slate-500" : "text-slate-400"}`}>({coldPercentage}%)</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className={`${
                      ...
                      <div className={`flex items-center justify-between pb-2.5 border-b mb-3 ${
                        ...
                        <span className={`text-[9.5px] font-mono ${isLight ? "text-slate-500" : "text-slate-400"}`}>0–200 Index</span>
                      </div>
                      ...
                      <div className="space-y-2.5">
                        ...
                        <div>
                          <div className="flex justify-between text-[10.5px] mb-1">
                            <span className={`font-semibold flex items-center gap-1.5 ${isLight ? "text-rose-700" : "text-rose-400"}`}>
                              <span>0–50</span>
                              <span className={`text-[8.5px] px-1.5 py-0.2 rounded font-medium ${
                                isLight ? "bg-rose-100 text-rose-700" : "bg-rose-500/10 text-rose-400"
                              }`}>Prime Targets</span>
                            </span>
                            <span className={`font-bold tabular-nums ${isLight ? "text-slate-900" : "text-white"}`}>{scorePoor} leads</span>
                          </div>
                          <div className={`h-1.5 w-full rounded-full overflow-hidden ${isLight ? "bg-slate-100" : "bg-slate-800/60"}`}>
                            <div 
                        ...
                        <div>
                          <div className="flex justify-between text-[10.5px] mb-1">
                            <span className={`font-semibold flex items-center gap-1.5 ${isLight ? "text-amber-700" : "text-amber-400"}`}>
                              <span>51–100</span>
                              <span className={`text-[8.5px] px-1.5 py-0.2 rounded font-medium ${
                                isLight ? "bg-amber-100 text-amber-700" : "bg-amber-500/10 text-amber-400"
                              }`}>Needs Work</span>
                            </span>
                            <span className={`font-bold tabular-nums ${isLight ? "text-slate-900" : "text-white"}`}>{scoreNeedsWork} leads</span>
                          </div>
                          <div className={`h-1.5 w-full rounded-full overflow-hidden ${isLight ? "bg-slate-100" : "bg-slate-800/60"}`}>
                            <div 
                        ...
                        <div>
                          <div className="flex justify-between text-[10.5px] mb-1">
                            <span className={`font-semibold flex items-center gap-1.5 ${isLight ? "text-indigo-700" : "text-indigo-400"}`}>
                              <span>101–150</span>
                              <span className={`text-[8.5px] px-1.5 py-0.2 rounded font-medium ${
                                isLight ? "bg-indigo-100 text-indigo-700" : "bg-indigo-500/10 text-indigo-400"
                              }`}>Moderate</span>
                            </span>
                            <span className={`font-bold tabular-nums ${isLight ? "text-slate-900" : "text-white"}`}>{scoreGood} leads</span>
                          </div>
                          <div className={`h-1.5 w-full rounded-full overflow-hidden ${isLight ? "bg-slate-100" : "bg-slate-800/60"}`}>
                            <div 
                        ...
                        <div>
                          <div className="flex justify-between text-[10.5px] mb-1">
                            <span className={`font-semibold flex items-center gap-1.5 ${isLight ? "text-emerald-700" : "text-emerald-400"}`}>
                              <span>151–200</span>
                              <span className={`text-[8.5px] px-1.5 py-0.2 rounded font-medium ${
                                isLight ? "bg-emerald-100 text-emerald-700" : "bg-emerald-500/10 text-emerald-400"
                              }`}>Established</span>
                            </span>
                            <span className={`font-bold tabular-nums ${isLight ? "text-slate-900" : "text-white"}`}>{scoreExcellent} leads</span>
                          </div>
                          <div className={`h-1.5 w-full rounded-full overflow-hidden ${isLight ? "bg-slate-100" : "bg-slate-800/60"}`}>
                            <div 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div 
                      ...
                      className={`p-3.5 rounded-xl border transition-all cursor-pointer hover:-translate-y-0.5 ${
                        isLight ? "bg-slate-50/60 border-slate-200 hover:border-indigo-300" : "bg-slate-900/60 border-slate-800 hover:border-indigo-500/40"
                      }`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className={`h-6 w-6 rounded-lg font-bold text-xs flex items-center justify-center border ${
                          isLight ? "bg-indigo-50 text-indigo-600 border-indigo-200" : "bg-indigo-600/10 text-indigo-400 border-indigo-500/20"
                        }`}>
                          1
                        </span>
                      ...
                      <p className={`text-[10px] mt-0.5 leading-relaxed ${isLight ? "text-slate-500" : "text-slate-400"}`}>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className={`h-6 w-6 rounded-lg font-bold text-xs flex items-center justify-center border ${
                          isLight ? "bg-violet-50 text-violet-600 border-violet-200" : "bg-violet-600/10 text-violet-400 border-violet-500/20"
                        }`}>
                          2
                        </span>
                      ...
                      <div className="mt-2 text-[10px] font-bold text-violet-500 flex items-center gap-1">
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className={`h-6 w-6 rounded-lg font-bold text-xs flex items-center justify-center border ${
                          isLight ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-emerald-600/10 text-emerald-400 border-emerald-500/20"
                        }`}>
                          3
                        </span>
            ...
            <div className="p-5 md:p-6 overflow-y-auto max-h-[calc(100vh-140px)] space-y-5">
              ...
              {pricingLoading && pricingPlans.length === 0 ? (
                <div className="flex items-center justify-center gap-2 py-12 text-[11px] text-slate-400">
                  ...
                  <span>Loading current plans…</span>
                </div>
              ) : pricingError && pricingPlans.length === 0 ? (
                <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 px-4 py-8 text-center text-[11px] text-rose-400">
                  {pricingError}
                </div>
              ) : pricingPlans.length === 0 ? (
                <div className={`rounded-xl border px-4 py-8 text-center text-[11px] ${isLight ? "border-slate-200 text-slate-500" : "border-[#1e293b] text-slate-400"}`}>
                  No plans are published right now. Contact us for current pricing.
                </div>
              ) : (
                <div
                  className={`grid grid-cols-1 gap-4.5 ${
                    pricingPlans.length >= 3 ? "md:grid-cols-3" : pricingPlans.length === 2 ? "md:grid-cols-2" : ""
                  }`}
                >
                  {pricingPlans.map((plan) => {
                    /*
                     * Current-plan detection matches the plan KEY, not the display
                     * name. The previous cards compared `planName` against the
                     * literals "free"/"pro"/"custom", so renaming a plan in the
                     * console silently stopped marking it as active.
                     */
                    const isCurrent = currentPlanKey
                      ? plan.key.toLowerCase() === currentPlanKey
                      : Boolean(planName) && plan.name.toLowerCase() === planName.toLowerCase();
                    // A zero price is "Free" for the free tier and a quote-only
                    // tier for anything else, which is how "Custom" is published.
                    const isQuoteOnly = plan.priceMonthly === 0 && plan.priceYearly === 0 && plan.key !== "free";
                    const isFree = plan.priceMonthly === 0 && !isQuoteOnly;
                    const featureRows = planFeatureRows(plan, pricingFeatureKeys, pricingFeatureLabels);

                    return (
                      <div
                        key={plan.key}
                        className={`border rounded-xl p-4.5 flex flex-col justify-between relative overflow-hidden transition-all duration-300 hover:scale-[1.01] ${
                          plan.highlight
                            ? isLight
                              ? "bg-blue-50/60 border-blue-200"
                              : "bg-gradient-to-b from-blue-950/40 to-black/35 border-blue-500/20"
                            : isLight
                              ? "bg-slate-50/50 border-slate-200"
                              : "bg-gradient-to-b from-[#111827] to-black/30 border-[#1e293b]"
                        } ${isCurrent ? "ring ring-emerald-500/40" : ""}`}
                      >
                        {(isCurrent || plan.highlight) && (
                          <div
                            className={`absolute top-2.5 right-2.5 text-[8px] font-black text-white px-1.5 py-0.5 rounded-full uppercase tracking-wider ${
                              isCurrent ? "bg-indigo-600" : "bg-blue-600"
                            }`}
                          >
                        ...
                        <div>
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{plan.name}</span>
                          <div className="mt-1 flex items-baseline">
                            <span className={`text-2xl font-bold tracking-tight ${isLight ? "text-slate-900" : "text-white"}`}>
                              {isQuoteOnly ? "Custom" : isFree ? "Free" : formatPlanPrice(plan.priceMonthly, plan.currency)}
                            </span>
                            {!isQuoteOnly && (
                              <span className="text-[10px] text-slate-500 ml-1">{isFree ? "forever" : "/mo"}</span>
                            )}
                          </div>
                          {plan.priceYearly > 0 && !isQuoteOnly && (
                            <div className="mt-0.5 text-[9.5px] text-slate-500">
                              or {formatPlanPrice(plan.priceYearly, plan.currency)}/yr
                            </div>
                          )}
                          {plan.description && (
                            <p className="mt-2 text-[10px] text-slate-450 leading-snug">{plan.description}</p>
                          )}
                          ...
                          <ul className="space-y-1.5 text-[10.5px] text-slate-400">
                            {featureRows.map((row) => (
                              <li key={row.label} className={`flex items-center gap-1.5 ${row.on ? "" : "opacity-30"}`}>
                                {row.on ? (
                        ...
                        {isCurrent ? (
                          <button
                            disabled
                            className="mt-6 w-full py-1.5 bg-emerald-600 text-white border-transparent text-[10px] font-bold rounded-lg cursor-not-allowed text-center"
                          >
                            Active Plan
                          </button>
                        ) : isQuoteOnly ? (
                          ...
                          <button
                            disabled
                            className="mt-6 w-full py-1.5 text-[10px] font-bold rounded-lg text-center cursor-not-allowed bg-slate-800/40 text-slate-500 border border-slate-700/20"
                          >
                            Included
                          </button>
                        ) : (
