...
/**
 * Authentication is applied per route below. Admin-only handlers use
 * requireAdmin directly so the dedicated console credential never passes a
 * general user-only guard, while user handlers remain ordinary-session-only.
 */
...
router.get("/analytics/user", requireAuth, async (req: Request, res: Response) => {
  try {
    if (!req.authUser) {
...
router.get("/notifications/count", requireAuth, async (req: Request, res: Response) => {
  try {
    if (!req.authUser) {
      return res.status(401).json({ error: "Authentication required." });
    }
