...
import { ADMIN_ENTITLEMENTS, type Entitlements, type GatedFeature } from "./plans";
...
export async function attachEntitlements(req: Request, _res: Response, next: NextFunction) {
  try {
    ...
    if (payload) {
      ...
      if (user) {
        req.authUser = user;
        // Read the operator-editable catalogue, so a quota or feature changed in
        // the console is enforced rather than only advertised. Falls back to the
        // compiled plans when the catalogue cannot be read.
        req.entitlements = await resolveEntitlements(user.plan);
      }
    }
  } catch (err) {
