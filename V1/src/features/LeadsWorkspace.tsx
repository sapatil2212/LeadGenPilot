    ...
    <div className="relative flex h-[calc(100vh-105px)] min-h-[580px] gap-3 overflow-hidden">
      ...
      <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} ${colors.panel} fixed bottom-0 left-0 top-0 z-40 w-64 overflow-y-auto border-r p-2.5 transition-transform md:static md:z-auto md:w-52 md:translate-x-0 md:rounded-xl md:border shadow-sm`}>
        ...
        <SidebarSection title="Leads">
          <SideNav icon={Database} label="All Leads" count={summary?.counts?.all} active={!hasFilters} onClick={() => chooseView({})} />
          <SideNav icon={Plus} label="New Leads" count={summary?.counts?.new} active={query.status === "NEW"} onClick={() => chooseView({ status: "NEW" })} />
          <SideNav icon={CircleUserRound} label="My Leads" count={summary?.counts?.mine} active={query.mine} onClick={() => chooseView({ mine: true })} />
          <SideNav icon={Flame} label="High Fit" count={summary?.counts?.highFit} active={query.fit === "HIGH"} onClick={() => chooseView({ fit: "HIGH" })} tone="text-rose-400" />
          <SideNav icon={Zap} label="Medium Fit" count={summary?.counts?.mediumFit} active={query.fit === "MEDIUM"} onClick={() => chooseView({ fit: "MEDIUM" })} tone="text-amber-400" />
          <SideNav icon={Snowflake} label="Low Fit" count={summary?.counts?.lowFit} active={query.fit === "LOW"} onClick={() => chooseView({ fit: "LOW" })} tone="text-sky-400" />
          <SideNav icon={CalendarClock} label="Recently Added" count={summary?.counts?.recent} active={query.sortBy === "recentlyAdded" && !hasFilters} onClick={() => chooseView({ sortBy: "recentlyAdded" })} />
        </SidebarSection>
        <SidebarSection title="Source">
          <SideNav icon={MapPin} label="Google Maps" count={summary?.counts?.sources?.GOOGLE_MAPS} active={query.source === "GOOGLE_MAPS"} onClick={() => chooseView({ source: "GOOGLE_MAPS" })} />
          <SideNav icon={Import} label="Imported" count={summary?.counts?.sources?.IMPORTED} active={query.source === "IMPORTED"} onClick={() => chooseView({ source: "IMPORTED" })} />
          {!!summary?.counts?.sources?.WEB_DISCOVERY && <SideNav icon={Globe2} label="Web Discovery" count={summary.counts.sources.WEB_DISCOVERY} active={query.source === "WEB_DISCOVERY"} onClick={() => chooseView({ source: "WEB_DISCOVERY" })} />}
          {!!summary?.counts?.sources?.GOOGLE_SHEETS && <SideNav icon={Database} label="Google Sheets" count={summary.counts.sources.GOOGLE_SHEETS} active={query.source === "GOOGLE_SHEETS"} onClick={() => chooseView({ source: "GOOGLE_SHEETS" })} />}
          {!!summary?.counts?.sources?.AI_DISCOVERED && <SideNav icon={Bot} label="AI Discovered" count={summary.counts.sources.AI_DISCOVERED} active={query.source === "AI_DISCOVERED"} onClick={() => chooseView({ source: "AI_DISCOVERED" })} />}
        </SidebarSection>
        <SidebarSection title="Status">
          {Object.entries(STATUS_LABELS).map(([value, label]) => <div key={value}><SideNav icon={value === "SUPPRESSED" ? ShieldOff : Check} label={label} count={summary?.counts?.statuses?.[value]} active={query.status === value} onClick={() => chooseView({ status: value })} /></div>)}
        </SidebarSection>
        <SidebarSection title="Outreach">
          <SideNav icon={Archive} label="Not Contacted" count={summary?.counts?.outreach?.NOT_CONTACTED} active={query.outreach === "NOT_CONTACTED"} onClick={() => chooseView({ outreach: "NOT_CONTACTED" })} />
          <SideNav icon={Mail} label="Email Available" count={summary?.counts?.outreach?.EMAIL_AVAILABLE} active={query.contact === "EMAIL"} onClick={() => chooseView({ contact: "EMAIL" })} />
          <SideNav icon={MessageSquare} label="WhatsApp Available" count={summary?.counts?.outreach?.WHATSAPP_AVAILABLE} active={query.contact === "WHATSAPP"} onClick={() => chooseView({ contact: "WHATSAPP" })} />
          <SideNav icon={Phone} label="Phone Available" count={summary?.counts?.outreach?.PHONE_AVAILABLE} active={query.contact === "PHONE"} onClick={() => chooseView({ contact: "PHONE" })} />
          <SideNav icon={MessageSquare} label="Replied" count={summary?.counts?.outreach?.REPLIED} active={query.outreach === "REPLIED"} onClick={() => chooseView({ outreach: "REPLIED" })} />
        </SidebarSection>
        <SidebarSection title="Actions">
          <SideNav icon={Filter} label="More Filters" onClick={() => setFiltersOpen(true)} />
...
function Modal({ title, isLight, onClose, children }: any) {
  return (
    <ModalPortal>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-xs p-4 animate-fadeIn" onMouseDown={onClose}>
        <div className={`w-full max-w-lg rounded-2xl border p-5 shadow-2xl relative animate-scaleUp ${isLight ? "bg-white border-slate-200 text-slate-900" : "bg-[#090d16] border-[#1e293b] text-white"}`} onMouseDown={(e) => e.stopPropagation()}>
