...
// The superadmin login and dashboard are exported by the unified Next.js app
// and are served with the other frontend routes after every API route is mounted.
...
async function startServer() {
  ...
  // Serve immutable Next.js chunks, public assets, and exported RSC payloads.
  // `index: false` keeps extensionless application routes in the resolver below
  // instead of relying on filesystem-dependent directory redirects.
  ...
  /**
   * Resolve a browser route to the exact HTML file emitted by `next build`.
   * APIs have already terminated above with JSON, so this fallback can never
   * turn an API typo into a successful HTML response.
   */
  app.get("*", (req, res, next) => {
    if (req.path.startsWith("/api/")) return next();

    const normalizedPath = req.path.replace(/\/+$/, "") || "/";
    const relativeRoute = normalizedPath === "/" ? "index" : normalizedPath.slice(1);

    // Refuse traversal and malformed encoded path segments before touching disk.
    if (!relativeRoute || relativeRoute.includes("..") || relativeRoute.includes("\\")) {
      return res.status(400).send("Invalid route.");
    }

    const candidates = normalizedPath === "/"
      ? [path.join(nextOutPath, "index.html")]
      : [
          path.join(nextOutPath, `${relativeRoute}.html`),
          path.join(nextOutPath, relativeRoute, "index.html"),
        ];
    const exportedPage = candidates.find((candidate) => fs.existsSync(candidate));

    if (exportedPage) {
      res.setHeader("Cache-Control", "no-cache");
      return res.sendFile(exportedPage);
    }

    const notFoundPage = path.join(nextOutPath, "404.html");
