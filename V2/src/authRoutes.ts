...
import { resolveEntitlements } from "./admin/planCatalog";
...
const ACCOUNT_SUSPENDED_MESSAGE =
  "This account has been suspended. Please contact support at leadgenpilot.in@gmail.com to restore access.";
...
router.get("/me", async (req: Request, res: Response) => {
  try {
    ...
    const allMemberships = await listMemberships(dbUser.id);
    const memberships = allMemberships.filter((membership) => membership.tenantStatus === "active");

    // Tenant suspension does not mutate User.status. Treat an account whose
    // memberships all point to suspended tenants as suspended at the auth
    // boundary, while still allowing access to any other active workspace.
    ...
    // Resolved from the operator-editable plan catalogue so the dashboard's plan
    // name, quota and feature locks match what the console publishes.
    const entitlements = await resolveEntitlements(dbUser.plan);
...
export async function requireAdmin(req: Request, res: Response, next: NextFunction) {
  const superAdminToken = req.cookies?.[env.auth.superAdminCookieName];
  const superAdminPayload = superAdminToken
    ? verifySuperAdminSessionToken(superAdminToken)
    : null;

  if (superAdminPayload && env.isSuperAdminConfigured()) {
    (req as any).user = superAdminPayload;
  ...
  // Resolve the ordinary credential independently. An expired dedicated cookie
  // must not veto a valid database-admin session in the same browser.
  const token = getSessionToken(req);
  const payload = token ? verifySessionToken(token) : null;
  ...
  if (payload?.sub === "superadmin") {
    if (
      ...
      if (remainingMs <= 0) {
        return res.status(401).json({ error: "Session expired.", code: "invalid_session" });
      }

      const migratedToken = issueSuperAdminSessionToken(payload.email, {
        expiresAt: payload.exp,
      });
      const migratedPayload = verifySuperAdminSessionToken(migratedToken);
      if (!migratedPayload) {
        return res.status(401).json({ error: "Session expired.", code: "invalid_session" });
      }

      res.cookie(
        env.auth.superAdminCookieName,
        migratedToken,
        superAdminSessionCookieOptions(remainingMs)
      );
      const clearOptions = { ...sessionCookieOptions(), maxAge: undefined };
      res.clearCookie(env.auth.cookieName, clearOptions);
      res.clearCookie("nexaleadai_session", clearOptions);
      (req as any).user = migratedPayload;
      (req as any).adminUser = { id: "superadmin", email: payload.email, role: "admin" };
      return next();
    }
    logger.warn(
      "Refused a legacy superadmin token because it is expired or the console is not fully configured."
    );
    return res.status(403).json({ error: "Administrator access required.", code: "not_admin" });
  }

  if (payload) {
    // Real database admins continue to authenticate with the ordinary user
    // cookie and are verified against the current database role.
    if (!env.isDatabaseConfigured()) {
  ...
  if (superAdminToken) {
    return res.status(superAdminPayload ? 403 : 401).json({
      error: superAdminPayload ? "Administrator access required." : "Session expired.",
      code: superAdminPayload ? "not_admin" : "invalid_session",
    });
  }
  if (token) {
    return res.status(401).json({ error: "Session expired.", code: "invalid_session" });
