...
describe("the routes Phase 4 locked down", () => {
  ...
  it.each([
    ...
    // pipeline moves to the database. requireAdmin performs authentication and
    // current-role authorization itself; placing requireAuth before it would
    // reject the dedicated superadmin cookie before the admin guard runs.
    const block = registration(method, routePath);
