    ...
    server: {
      ...
      watch: process.env.DISABLE_HMR === 'true' ? null : {
        ignored: [
          ...
          '**/.env',
          '**/src/config.ts',
          // The superadmin console is plain static HTML/CSS/JS served straight
          // from public/, outside the React module graph entirely. Vite still
          // watches everything under publicDir and fires a full-page reload on
          // any change there — including mid-edit saves — which raced the admin
          // console's own boot sequence (auth check -> render shell -> route)
          // and could reload the tab before it finished, leaving nothing on
          // screen but the auth-gate spinner. A plain browser refresh already
          // picks up static asset changes, so Vite reloading it too is pure
          // downside for this subtree.
          '**/public/superadmin-assets/**',
          '**/public/superadmin-dashboard.html',
