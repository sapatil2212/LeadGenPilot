...
import authRoutes, { requireAdmin } from "./src/authRoutes";
...
import publicPlansRoutes from "./src/publicPlansRoutes";
...
// ── Published pricing (public, read-only) ──
// Mounted before the API-key guard because visitors reading the pricing section
// have neither a key nor a session. Rate limited since it is unauthenticated.
app.use("/api/plans", apiRateLimiter(), publicPlansRoutes);
 ...
 */
app.get("/api/failed", requireAdmin, (req, res) => {
...
app.post("/api/retry-failed", requireAdmin, async (req, res) => {
  try {
    let customWebhookUrl: string | undefined;
 ...
 */
app.post("/api/reset-data", requireAdmin, (req, res) => {
  try {
    const processedPath = path.join(process.cwd(), "processed-leads.json");
...
async function startServer() {
  ...
  // ── Company & Legal static pages (About, Privacy, Terms, Cookies) ──
  const companyPages = [
    { routes: ["/about", "/about-us", "/about/", "/about-us/"], file: "about" },
    { routes: ["/privacy", "/privacy-policy", "/privacy/", "/privacy-policy/"], file: "privacy" },
    { routes: ["/terms", "/terms-of-service", "/terms/", "/terms-of-service/"], file: "terms" },
    { routes: ["/cookies", "/cookie-policy", "/cookies/", "/cookie-policy/"], file: "cookies" },
  ];

  for (const { routes, file } of companyPages) {
    app.get(routes, (_req, res, next) => {
      const candidates = [
        path.join(nextOutPath, `${file}.html`),
        path.join(nextOutPath, file, "index.html"),
        path.join(process.cwd(), "public", `${file}.html`),
      ];
      const match = candidates.find((p) => fs.existsSync(p));
      if (match) return res.sendFile(match);
      next();
    });
  }
